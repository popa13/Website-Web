    ...
    <div class="salon-side-bottom">
      ...
      <div class="mr-panel">
        ...
        <div class="salon-mode-row">
          ...
          <button class="salon-mode-btn" id="salon-mode-svg">Vector</button>
        </div>
...
<div id="salon-tuto-panel" style="display:none;">
  <div id="salon-tuto-header">
    ...
    <button id="salon-tuto-close" aria-label="Close">✕</button>
  </div>
  <div id="salon-tuto-bar"><div id="salon-tuto-fill"></div></div>
  <h3 id="salon-tuto-title"></h3>
  <p id="salon-tuto-text"></p>
  <div id="salon-tuto-nav">
    <button class="salon-btn" id="salon-tuto-prev">← Back</button>
    <button class="salon-btn salon-btn-primary" id="salon-tuto-next">Next →</button>
  </div>
</div>
...
const RING_R = 28; // radius of the rotation ring in canvas pixels
...
function render() {
  ...
  if (isHoveringCenter || isRotating) {
    ...
    // 270° CCW arc starting from horizontal, with arrowhead
...
function renderSVG() {
  ...
  if (!svgLeaves) {
    ...
    fractalSVG.innerHTML = `<text x="${cx}" y="${cy}" text-anchor="middle" dominant-baseline="middle" font-size="${fs}" fill="#bbb">Init → Iterate</text>`;
    svgVisibleCount = 0;
    return;
  }
...
/* Enlarged touch rotation zone for fingers (≈34 screen px) */
...
function updateIterLabel() {
  ...
  document.getElementById('salon-iter-label').textContent =
    hasData ? `Iteration: ${iterCount}` : 'Iteration: —';
...
function updateCanvasInfo() {
  ...
  if (renderMode === 'svg') {
    ...
    const vis = zoomW < 0.999 ? ` (${svgVisibleCount} visible)` : '';
    document.getElementById('salon-canvas-info').textContent = `${n} shape${n !== 1 ? 's' : ''}${vis}${zoom}`;
  } else {
...
document.getElementById('salon-mode-bitmap').addEventListener('click', () => {
  ...
  document.getElementById('salon-btn-save-png').textContent = '↓ Save PNG';
...
document.getElementById('salon-mode-svg').addEventListener('click', () => {
  ...
  document.getElementById('salon-btn-save-png').textContent = '↓ Save SVG';
...
document.getElementById('salon-btn-save-png').addEventListener('click', () => {
  if (renderMode === 'svg') {
    if (!svgLeaves) { alert('No fractal to save. Run an iteration first.'); return; }
    ...
    a.download = `ifs-fractal-iter${iterCount}.svg`;
    a.href = url; a.click();
...
const TUTO = [
  ...
  { title: 'Step 5 — Iterate',
    text: 'The fractal canvas shows a filled square. Click "Iterate ×1" to apply all 3 maps. Keep clicking to watch the Sierpiński triangle emerge!',
  ...
  { title: 'Going Further — A Fractal with Rotations',
    text: 'Let\'s build a new fractal at scale ½ that uses rotations. Hover over the center of the green square to reveal the rotation ring and its handle dot.',
    hl: ['#salon-canvas'] },
  { title: 'Transformation 1 — Bottom-left corner, 0°',
    text: 'The square is placed at the bottom-left corner (0.25, 0.25) with no rotation — the dot on the ring sits at the horizontal (3 o\'clock). Click "+ Add" to record T1.',
    hl: ['#salon-btn-add', '#salon-canvas'] },
  { title: 'Transformation 2 — Top-left corner, 270°',
    text: 'The square has moved to the top-left corner (orange arrow). Hover over its center to reveal the ring, grab the dot and drag it counter-clockwise until the counter reads 270°. Then click "+ Add".',
    hl: ['#salon-btn-add', '#salon-canvas'] },
  { title: 'Transformation 3 — Bottom-right corner, 90°',
    text: 'The square has moved to the bottom-right corner (orange arrow). Do the same: hover over the center, grab the dot on the ring and drag it counter-clockwise until 90° is shown. Click "+ Add".',
    hl: ['#salon-btn-add', '#salon-canvas'] },
...
function tutoApply(step) {
  ...
  function setList(list) {
    transforms = list.map(t => ({ ...t })); editingIndex = -1; btnAdd.textContent = '+ Add';

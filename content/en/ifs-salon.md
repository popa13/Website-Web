...
let R          = 512;   // internal resolution (pixels)
...
/* ── Background drawing (dashed centre lines only) ── */
function drawGrid() {
...
function renderFractal() {
  ...
  if (iterBitmap) {
    ...
    fractalCtx.fillText('Press ⊙ Init then Iterate', S/2, S/2);
  }
}
...
function updateCanvasInfo() {
  ...
  document.getElementById('salon-canvas-info').textContent =
    `Internal resolution ${R}×${R} px — preview ${S}×${S} px ${ratio}`;
}
...
document.getElementById('salon-btn-share-email').addEventListener('click', () => {
  ...
  const body = encodeURIComponent(
    ...
    'Click the link below to view and explore it interactively:\n\n' +
    url + '\n\nBest regards'
  );

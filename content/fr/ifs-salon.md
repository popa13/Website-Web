...
<p class="subtitle">
  ...
  \( f_i(\mathbf{x}) = \tfrac{1}{2}\,R_\theta\,\mathbf{x} + \mathbf{b}_i \)
...
<div class="salon-app">
  ...
  <aside class="salon-sidebar">
    ...
    <button class="salon-btn salon-btn-email" id="salon-btn-email">✉ Envoyer par courriel</button>
  ...
  <div class="salon-canvas-col" id="salon-canvas-col">
    ...
    <p class="salon-hint">
      Glissez le carré bleu · <span class="salon-red">arête rouge</span> = repère de rotation ·
      Le point rouge est l'image du coin&nbsp;(1,&nbsp;1).
    </p>
...
<div id="salon-modal-overlay" class="salon-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="salon-modal-title">
  <div class="salon-modal-box">
    <p class="salon-modal-title" id="salon-modal-title">Envoyer la fractale par courriel</p>
    <p class="salon-modal-note">
      L'image PNG sera téléchargée sur votre appareil. Votre client de messagerie s'ouvrira
      avec le destinataire et un message pré-remplis — veuillez joindre le fichier téléchargé.
    </p>
    <input id="salon-email-input" class="salon-modal-input" type="email"
           placeholder="destinataire@exemple.com" autocomplete="email" />
    <div class="salon-modal-actions">
      <button class="salon-btn salon-btn-sm" id="salon-modal-cancel">Annuler</button>
      <button class="salon-btn salon-btn-sm salon-btn-email" id="salon-modal-send">Envoyer</button>
    </div>
  </div>
</div>
...
fractalCanvas.width = fractalCanvas.height = S;
...
let transforms   = [];
let editingIndex = -1;
let cur = { cx: H, cy: H, rotation: 0, flip: 'none' };
let isDragging = false;
let dragOff    = { x: 0, y: 0 };

let R          = 512;   // résolution interne (pixels)
...
function getCorners(cx, cy, rotation, flip) {
...
function getAffine(t) {
  ...
  let a, b, cc, d;
  if (t.flip === 'H') {
    a = SCALE*c;  b =  SCALE*si;
    cc= SCALE*si; d = -SCALE*c;
  } else if (t.flip === 'V') {
    a = -SCALE*c;  b = -SCALE*si;
    cc= -SCALE*si; d =  SCALE*c;
  } else {
...
/* ── Dessin du fond (lignes pointillées uniquement) ── */
function drawGrid() {
...
function drawSquare(cx, cy, rotation, flip, pal, alpha, label) {
  const pts  = getCorners(cx, cy, rotation, flip);
...
function render() {
  ...
  transforms.forEach((t, i) =>
    drawSquare(t.cx, t.cy, t.rotation, t.flip, PALETTE[i % PALETTE.length], 0.85, `T${i+1}`)
  );
...
function renderFractal() {
  fractalCtx.clearRect(0, 0, S, S);
  if (iterBitmap) {
    fractalCtx.save();
    fractalCtx.imageSmoothingEnabled = false;
    fractalCtx.drawImage(offCanvas, 0, 0, S, S);
    fractalCtx.restore();
  } else {
    // Placeholder
    fractalCtx.fillStyle = '#fafafa';
    fractalCtx.fillRect(0, 0, S, S);
    fractalCtx.strokeStyle = '#ccc';
    fractalCtx.setLineDash([6, 4]);
    fractalCtx.strokeRect(1, 1, S-2, S-2);
    fractalCtx.setLineDash([]);
    fractalCtx.fillStyle = '#bbb';
    fractalCtx.font = `${Math.round(S*0.035)}px sans-serif`;
    fractalCtx.textAlign = 'center';
    fractalCtx.textBaseline = 'middle';
    fractalCtx.fillText('Appuyez sur ⊙ Init puis Itérer', S/2, S/2);
  }
}
...
function updateCanvasInfo() {
  const ratio = R > S ? `(÷${R/S})` : R < S ? `(×${S/R})` : '(1:1)';
  document.getElementById('salon-canvas-info').textContent =
    `Résolution interne ${R}×${R} px — aperçu ${S}×${S} px ${ratio}`;
}
...
function refreshList() {
  ...
  transforms.forEach((t, i) => {
    ...
    div.innerHTML = `
      ...
      <div class="salon-t-info">s = 0.5 &nbsp;·&nbsp; rot = ${t.rotation}°${t.flip !== 'none' ? ' &nbsp;·&nbsp; ' + t.flip : ''}<br>c = (${t.cx.toFixed(3)}, ${t.cy.toFixed(3)})</div>
...
document.getElementById('salon-btn-reset').addEventListener('click', () => {
  ...
  cur = { cx: H, cy: H, rotation: 0, flip: 'none' };
  document.querySelectorAll('.salon-rot-btn').forEach(b =>
  ...
  btnAdd.textContent = '+ Ajouter';
  resetIter(); updatePosLabel(); refreshList(); render();
});
...
document.getElementById('salon-btn-email').addEventListener('click', () => {
  if (!iterBitmap) { alert('Aucune fractale à envoyer. Lancez d\'abord une itération.'); return; }
...
document.getElementById('salon-modal-send').addEventListener('click', async () => {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    alert('Veuillez entrer une adresse courriel valide.'); return;
  }
  const imageData = offCanvas.toDataURL('image/png');
  if (imageData.length > 8 * 1024 * 1024) {
    alert('Image trop grande pour l\'envoi. Veuillez choisir une résolution plus petite.'); return;
  }
  ...
  try {
    ...
    if (resp.ok && result.ok) {
      ...
      alert('Erreur : ' + (result.error || 'Impossible d\'envoyer le courriel.'));
    }
  } catch (e) {
    alert('Erreur réseau. Impossible d\'envoyer le courriel.');
  } finally {
